...
let make_inverse_palette (palette : color list) : color PaletteMap.t = 
  let indexed_palette = List.mapi (fun index col -> (index, col)) in
  let map : color PaletteMap.t = PaletteMap.empty in
    List.fold_left (fun index col -> PaletteMap.add col index map) map indexed_palette;
...
let tick (t : int) (screen : screen) =
  ...
  let fx = 120. +. (sin ((2.28 +. (0.2 *. cos (ft /. 100.))) *. ft /. 80.) *. 100.)
  and fy = 68. +. (cos ((3.7 +. (0.4 *. sin (ft /. 10.))) *. ft /. 80.) *. 60.) in
